...
export function generateStaticParams() {
  return listPublicGuides('pt-BR').map((guide) => ({ slug: guide.slug }))
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const guide = getPublicGuideBySlug('pt-BR', slug)
  if (!guide) {
    return {}
  }
...
export default async function GuidePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const relatedGuides = guide.relatedIds
    .map((id) => getPublicGuideById('pt-BR', id))
  ...
  return (
    <>
      ...
      >
        <PublicGuideContent guide={guide} locale="pt-BR" />

        {relatedGuides.length ? (
          <section className="mt-8 rounded-3xl border border-white/10 bg-surface/55 p-6 md:p-8">
            <h2 className="text-2xl font-bold text-white">Guias relacionados</h2>
