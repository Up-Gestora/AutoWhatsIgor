...
export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ chatId: string; mediaRef: string }> }
) {
  const { chatId, mediaRef } = await params
  const safeChatId = chatId.trim()
  const safeMediaRef = mediaRef.trim()
  ...
  const url = new URL(
    `${backendUrl}/sessions/${encodeURIComponent(auth.sessionId)}/chats/${encodeURIComponent(safeChatId)}/messages/${encodeURIComponent(safeMediaRef)}/media`
  )
  ...
  if (!response.ok) {
    ...
    return NextResponse.json({ error: 'backend_request_failed' }, { status: response.status || 502 })
  }
