...
const normalizeClientStatus = (value: unknown) => {
  const normalized = typeof value === 'string' ? value.trim().toLowerCase() : ''
  if (normalized === 'ativo' || normalized === 'inativo' || normalized === 'vip' || normalized === 'lead') {
    return normalized
  }
  return 'ativo'
}
...
const loadClientsFromFirestore = async (
  sessionId: string,
  limit: number
): Promise<FirestoreClientsResult | null> => {
  if (!adminDb) {
    return null
  }

  const collection = adminDb.collection('users').doc(sessionId).collection('clientes')
  ...
  const clients = snapshot.docs.map((doc) => {
    const data = (doc.data() ?? {}) as FirestoreClientData
    return {
      id: doc.id,
      ...
      lastContactAt: toMillis(data.lastContact),
      nextContactAt: toMillis(data.nextContact),
      ...
      totalValue: toNumberOrNull(data.totalValue)
    }
  })

  clients.sort((a, b) => {
    const av = a.lastContactAt ?? 0
    const bv = b.lastContactAt ?? 0
    return bv - av
  })
...
export async function GET(request: NextRequest) {
  try {
    ...
    const tryFirestoreFallback = async () => {
      ...
      const firestoreClients = await loadClientsFromFirestore(auth.sessionId, safeLimit)
      if (firestoreClients) {
        return NextResponse.json({
          success: true,
          clients: firestoreClients.clients,
          total: firestoreClients.total,
    ...
    if (!backendUrl || !adminKey) {
      if (hasSearch) {
        return NextResponse.json(
          {
            error: 'client_search_unavailable_in_fallback',
    ...
    try {
      response = await fetchBackendWithFallback(
        backendUrls,
        `/sessions/${encodeURIComponent(auth.sessionId)}/clients${query.toString() ? `?${query}` : ''}`,
        {
          headers: {
      ...
      if (hasSearch) {
        ...
        return NextResponse.json(
          { error: 'client_search_unavailable_in_fallback', detail },
          { status: 503 }
        )
      }
      const fallback = await tryFirestoreFallback()
    ...
    if (!response.ok) {
      ...
      if (hasSearch) {
        if (error === 'client_search_too_short') {
    ...
    const payloadRecord =
      payload && typeof payload === 'object' ? (payload as Record<string, unknown>) : null
    if (payloadRecord && Array.isArray(payloadRecord.clients)) {
    ...
    return NextResponse.json({ error: 'clients_proxy_failed', detail }, { status: 502 })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = (await request.json().catch(() => ({}))) as ClientCreateBody
    const payload: ClientCreateBody = {}
    ...
    if (!payload.name && !payload.whatsapp) {
      return NextResponse.json({ error: 'client_create_required' }, { status: 400 })
    }
    ...
    const response = await fetchBackendWithFallback(
      backendUrls,
      `/sessions/${encodeURIComponent(auth.sessionId)}/clients`,
      {
        method: 'POST',
        headers: {
          'x-admin-key': adminKey,
