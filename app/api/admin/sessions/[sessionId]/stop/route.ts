...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const response = await fetch(`${backendUrl}/sessions/${encodeURIComponent(sessionId)}/stop`, {
    method: 'POST',
  ...
  if (!response.ok) {
    return NextResponse.json(payload ?? { error: 'backend_stop_failed' }, { status: response.status })
  }
