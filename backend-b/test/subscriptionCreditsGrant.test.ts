...
test('CreditsService.grantSubscriptionCredits grants when amountBrl > 0', async () => {
  const store = new FakeCreditsStore()
  const service = new CreditsService({ store: store as any })

  const result = await service.grantSubscriptionCredits('sess_1', 20)

  assert.equal(result.granted, true)
  assert.equal(result.credits.balanceBrl, 20)
  assert.equal(store.adjustCalls.length, 1)
  assert.equal(store.adjustCalls[0]?.meta.source, 'stripe_subscription')
  assert.equal(store.adjustCalls[0]?.meta.reason, 'subscription_credits')
  assert.equal(store.adjustCalls[0]?.meta.actorId, 'stripe')
  assert.equal(store.adjustCalls[0]?.meta.referenceId, null)
})

test('CreditsService.grantSubscriptionCredits is idempotent on unique violation', async () => {
  const store = new FakeCreditsStore()
  store.failWithDuplicate = true
  const service = new CreditsService({ store: store as any })

  const result = await service.grantSubscriptionCredits('sess_1', 20, { referenceId: 'in_1' })
...
test('CreditsService.grantSubscriptionCredits skips when amountBrl <= 0', async () => {
  const store = new FakeCreditsStore()
  const service = new CreditsService({ store: store as any })

  const result = await service.grantSubscriptionCredits('sess_1', 0)
