...
import { type TutorialBlock, type TutorialSection, type TutorialTopic as Topic } from '@/lib/tutorials/content'
...
function BlockRenderer(props: { block: TutorialBlock; toggleStatusByKey?: Record<string, string> }) {
  ...
  if (block.type === 'toggleCards') {
    ...
    const statusStyles = (status: string) => {
      if (status === 'Ligado' || status === 'Encaminhar') return 'bg-primary/10 text-primary border-primary/25'
      if (status === 'Desligado' || status === 'Silencio' || status === 'Silêncio') return 'bg-surface-lighter/40 text-gray-300 border-surface-lighter'
      if (status === 'Misto') return 'bg-amber-500/10 text-amber-300 border-amber-500/30'
      if (status === 'Em breve' || status === 'Sem dados' || status === 'Sem agenda' || status === 'No painel') {
...
export function TutorialTopic(props: {
  topic: Topic
  completed: boolean
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 shrink-0">
              ...
              >
                ...
                {props.completed ? 'Marcar como não concluído' : 'Marcar como concluído'}
              </Button>
            </div>
          </div>
        </div>
